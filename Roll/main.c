...
#include "sct.h"
#include <stdlib.h> // Include for randomization function
...
#define START_TIME		40	// 40ms as a start time for the delay when showing the "throwing" effect
#define STOP_TIME		1000 // 1000ms as a stop time for the delay

#define DEBOUNCE_TIME	40 //40ms for debounce
/* USER CODE END PD */
...
int main(void)
{
  ...
  while (1)
  {
	...
	static EFFECT_STATUS do_effect = EFFECT_OFF;
	...
	if (HAL_GPIO_ReadPin(S2_GPIO_Port, S2_Pin) != GPIO_PIN_RESET) {
		// If S2 has been pressed
		// Show the random value immediately
		do_effect = EFFECT_OFF; // If the "throwing" effect is running, stop

		rand_value = rand() % (MAX_RAND_NUM - MIN_RAND_NUM + 1) + MIN_RAND_NUM; // Generate random number in interval

		sct_value(rand_value);
	...
	if(s1_push_time && HAL_GetTick() - s1_push_time > DEBOUNCE_TIME) {
		if(HAL_GPIO_ReadPin(S1_GPIO_Port, S1_Pin) == GPIO_PIN_RESET) {
			if(s1_state == GPIO_PIN_RESET) {
				...
				// Generate random value and perform the "throwing" effect
				rand_value = rand() % (MAX_RAND_NUM - MIN_RAND_NUM + 1) + MIN_RAND_NUM; // Generate random number in interval <1,6>
	...
	if (do_effect) {
		// Generate random number in interval <1, 6> to "simulate" the throwing
		uint8_t rand_throw_value = rand() % (MAX_RAND_NUM - MIN_RAND_NUM + 1) + MIN_RAND_NUM;
		static uint8_t delay_time = START_TIME;

		sct_value(rand_throw_value);

		HAL_Delay(delay_time += 100); //Every loop increase the delay by 100ms

		if(delay_time >= STOP_TIME) {
			sct_value(rand_value);
			do_effect = EFFECT_OFF;
